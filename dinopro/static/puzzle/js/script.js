...
let END_TIME = null;
let nameImageList = [];
// let randomImage = none;

let POP_SOUND = new Audio('../static/puzzle/source/pop.mp3');
...
function main() {
  ...
  var randomImage = Math.floor(Math.random() * nameImageList.length);
  PUZZLE_IMAGE = document.createElement('img');
  PUZZLE_IMAGE.src = `../static/puzzle/images/puzzle/${randomImage}.jpg`;

  PUZZLE_IMAGE.onload = function () {
    console.log('WINDOW.ONLOADE');
    handleResize();
    initializePieces(SIZE.rows, SIZE.columns);
    updateGame();
    console.log(PUZZLE_IMAGE);
  };
}

function genNameImage(number) {
  nameImageList = [];
  let count = 1;
  while (count <= number) {
    nameImageList.push(String(count));
    count++;
  }
}
...
function handleResize() {
  ...
  let resizer =
    SCALER *
    Math.min(
      window.innerWidth / PUZZLE_IMAGE.width,
      window.innerHeight / PUZZLE_IMAGE.height
    );
  SIZE.width = resizer * PUZZLE_IMAGE.width;
  SIZE.height = resizer * PUZZLE_IMAGE.height;
...
function updateGame() {
  ...
  CONTEXT.drawImage(PUZZLE_IMAGE, SIZE.x, SIZE.y, SIZE.width, SIZE.height);
...
class Piece {
  ...
  draw(context, useCam = true) {
    ...
    const scaledTabHeight =
      (Math.min(
        PUZZLE_IMAGE.width / SIZE.columns,
        PUZZLE_IMAGE.height / SIZE.rows
      ) *
        tabHeight) /
      sz;
    ...
    if (useCam) {
      context.drawImage(
        PUZZLE_IMAGE,
        (this.colIndex * PUZZLE_IMAGE.width) / SIZE.columns - scaledTabHeight,
        (this.rowIndex * PUZZLE_IMAGE.height) / SIZE.rows - scaledTabHeight,
        PUZZLE_IMAGE.width / SIZE.columns + scaledTabHeight * 2,
        PUZZLE_IMAGE.height / SIZE.rows + scaledTabHeight * 2,
        this.x - tabHeight,
        this.y - tabHeight,
...
function showEndScreen() {
  ...
  const timeView = formatTime(END_TIME - START_TIME);
  document.getElementById('scoreValue').innerHTML = 'Score: ' + timeView;
